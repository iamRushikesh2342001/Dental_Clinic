  ...
  if(isset($_POST['login_user'])){
    if(!isset($_SESSION['username']))
    {
      ...
      if(!empty($username) && !empty($password1))
      {
        ...
        $query = " SELECT * FROM user WHERE username='$username' and password='$password1' and role = '$role' LIMIT 1";
        $results = mysqli_query($db, $query);
        $res = mysqli_fetch_assoc($results);
        if($username == $res['username'] && $password1==$res['password'] && $role == $res['role'])
        {
    ...
    <div class="container">
        <div class="signin-content">
            <div class="signin-image">
                ...
                <a href="registration.php" class="signup-image-link">Don't have an account? Sign Up</a>
            </div>
