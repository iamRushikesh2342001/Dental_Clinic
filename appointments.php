  ...
  <?php require_once("header.php");?>
    ...
    <?php
        if($_SESSION['role'] == 'patient') {
            ...
            {
                echo "<table><tr><th>Dentist</th><th>Location</th><th>Date</th><th>Time</th><th>Purpose</th><th>Status</th><th>Action</th></tr>";
        ...
        elseif ($_SESSION['role'] == 'dentist') {
            ...
            {
                echo "<table><tr><th>Patient Name</th><th>Date</th><th>Time</th><th>Purpose</th><th>Status</th><th>Action</th></tr>";
                while($row = mysqli_fetch_assoc($result))
                {
                    if(date('Y-m-d') < $row['date'])
                    {
                        ...
                        if($row['status'] == 'Pending')
                        {
                            ...
                            echo "<td><a href='".$url1."' style='color:green'>Confirm</a>";
                            echo "<br><br><form action='' method='post'><a href='".$url2."' style='color:red'>Reject</a></td></tr>";
                        }
        ...
        elseif ($_SESSION['role'] == 'admin') {
            ...
            if ($result == false || mysqli_num_rows($result) == 0) {
                ...
                echo "<table><tr><th>Patient Name</th><th>Dentist</th><th>Location</th><th>Date</th><th>Time</th><th>Status</th></tr>";
        
                while ($row = mysqli_fetch_assoc($result)) {
                    if(date('Y-m-d') < $row['date'])
                    {
                        $patientQuery = "SELECT name FROM useraccount WHERE username = '".$row['uname']."'";
                        $dentistQuery = "SELECT name FROM dentist WHERE username = '".$row['dname']."' LIMIT 1";
                        $patientResult = mysqli_query($db, $patientQuery);
                        $dentistResult = mysqli_query($db, $dentistQuery);
                        $patientRow = mysqli_fetch_assoc($patientResult);
                        $dentistRow = mysqli_fetch_assoc($dentistResult);
                    
                        echo "<tr>";
                        echo "<td>".$patientRow['name']."</td>";
                        echo "<td>".$dentistRow['name']."</td>";
